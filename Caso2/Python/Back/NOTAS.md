...
| `main.py` | Lee un archivo de comandos (formato `PARQUE` + lista de ascensores + `---` + comandos) y por cada línea imprime `> <comando>` seguido del resultado o del rechazo. Expone `cargar_archivo()` y `procesar_linea()` para que el Front las reutilice sin reimplementar el despacho de comandos. |
